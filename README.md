...
This project is a backend service that fetches cryptocurrency prices from the CoinGecko API and exposes them through REST endpoints.
The system is designed to handle high traffic efficiently by batching concurrent requests for the same coin.
...
5. Batch is triggered when:
   - 3 requests arrive, or
   - 5 seconds pass
6. A single call is made to CoinGecko
7. Result is saved to PostgreSQL
8. All waiting requests receive the same response

This reduces unnecessary external API calls.
